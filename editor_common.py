...
class EditFile:
    ...
    def __copy__(self):
        ...
        result.working = None
        if self.working:
            result.working = open(self.working.name,"r")
        return result
    ...
    def open(self):
        ...
        if os.path.exists(self.filename):
            self.working = open(os.path.abspath(self.filename),"r")
        elif not self.readonly:
            self.working = open(os.path.abspath(self.filename),"w+")
        else:
...
#                            print >>open("ped.log","a"),traceback.format_exc()
            ...
            for key,value in self.__dict__.items():
                print >>log,key,"=",value,"len(",len(value) if hasattr(value,'__len__') else 0,")"
            raise
