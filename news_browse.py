...
#        scr.nooutrefresh()
    ...
    def newmessage(self):
        ...
        if values:
            ...
            mesg = StringIO.StringIO()
            groups = newmessage.groups.getvalue()
            server = self.server_from_groups( groups )
            print >>mesg,"Newsgroups:",groups
            print >>mesg,"Subject:",newmessage.subject.getvalue()
            print >>mesg,"From:",server.email_addr
            print >>mesg,""
            print >>mesg, open(newmessage.new_message,"r").read()
            mesg.seek(0)
    ...
    def replymessage(self):
        ...
        if self.current_message:                                 
            ...
            if values:
                ...
                print >>mesg,"Newsgroups:",groups
                print >>mesg,"Subject:",replymessage.subject.getvalue()
                print >>mesg,"From:",server.email_addr
                references = ""
                ...
                print >>mesg,"References:",references
                
                print >>mesg,""
                print >>mesg, open(replymessage.new_message,"r").read()
                mesg.seek(0)
                self.getConnection(server).post(mesg)
                mesg.close()
