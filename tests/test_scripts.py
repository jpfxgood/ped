...
def test_scripts ( request ):
    ...
    exec_path = os.path.join(os.path.dirname(os.path.dirname(request.fspath)),"scripts")
    hex_output = open(os.path.join(os.path.dirname(request.fspath),"hex_dump.txt"),"r",encoding="utf-8",newline='').read()
    child = pexpect.spawnu(os.path.join(exec_path,'hex')+' '+os.path.join(exec_path,'hex'))
    cmd_output = child.read()
    assert(cmd_output == hex_output)
    assert(child.isalive() == False)

    child = pexpect.spawnu(os.path.join(exec_path,'pyfind')+' -p "import\s*editor" '+os.path.join(exec_path,'ped'))
    cmd_output = child.read()
    assert(cmd_output == pyfind_output)
    assert(child.isalive() == False)

    child = pexpect.spawnu(os.path.join(exec_path,'ped'))
    ...
    child = pexpect.spawnu(os.path.join(exec_path,'pless')+' '+os.path.join(exec_path,'pless'))
    assert(child.isalive() == True)
